...
>*The greenness would slowly increase post-fire as the fynbos begins to recover but after a certain stage the plants will not get any green hence the saturation. Greenness of plants does not change at a constant rate per time interval so the trend could not be linear but rather there is a faster increase in greenness early post-fire but this rate decreases with time and later tapers off as plants mature.*
<br>
...
>*The alpha, gamma and lambda estimates were not very different in the two models. The alpha and lambda estimates were smaller in the full model whereas gamma was estimated to be larger than it was estimated in the simple model.  lambda is the growth rate *
<br>
...
>*The full model is the better model according to the ANOVA since it has a lower residual sum of squares and thus fits the data better.*
Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>*There are two degrees of freedom in the ANOVA because that is the difference in the number of parameters between the two models *
<br>
...
>*The full model is the preferred model because it has the lower AIC value which means it has a lower prediction error relative to the simple model for this given data set*
<br>
...
>*Though AIC can be computed for NLE models the method is for calculating AIC by applying the log-likelihood function does not return the same result as when the method is applied to MLE models thus it would not be fair to compare them.*
<br>